...
function displayGraph() {
	...
	for (var i = 0; i < event.choices.length; i++){
	    ...
	    if (next) {
		g.addNode(eventKey, {render:renderFirst});
		g.addNode(next, {render:renderGeneric});
		g.addEdge(eventKey, next, {directed:true});
	    }
	}
    });
    for (var eventKey in g.nodes) {
	var hasNext = false;
	// Style events that do not exist at all.
	if (!events[eventKey]) {
	    g.nodes[eventKey].render = renderNoExist;
	    continue;
	}
	...
	if (!hasNext) {
	    g.nodes[eventKey].render = renderNoChoice;
	}
    }
